        ...
        for record in todo:
            ...
            for slot in range(args.n_per_example):
                ...
                for _outer in range(outer_attempts):
                    ...
                    candidate, _elapsed, _used_fallback, opening_text = generate_variant(
                    ...
                    reason = validation_reason(candidate, mapping, [t for t, _s in accepted])
                    ...
                    if reason is None:
                        accepted.append((candidate, style_tag))
                        tracker.remember(style_tag, candidate)
                        break
                    else:
                        log_rejected(1, f"{slot_label} (combined)", reason, candidate)
                        similar_text, ratio = find_similar_text(candidate, [t for t, _s in accepted])
                        if similar_text:
            ...
            final_entries = [(unmask(text, mapping), style_tag) for text, style_tag in accepted]
